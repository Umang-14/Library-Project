...
public class TransactionsFrame extends JFrame {
    ...
    public TransactionsFrame(String username, boolean isAdmin) {
        ...
        availBtn.addActionListener(e -> { dispose(); new BookAvailableFrame(username, isAdmin).setVisible(true); });
        issueBtn.addActionListener(e -> { dispose(); new BookIssueFrame(username, isAdmin).setVisible(true); });
        returnBtn.addActionListener(e -> { dispose(); new ReturnBookFrame(username, isAdmin).setVisible(true); });
        fineBtn.addActionListener(e -> { dispose(); new PayFineFrame(username, isAdmin).setVisible(true); });
