...
public class UserManagementFrame extends JFrame {
    ...
    public UserManagementFrame(String username, boolean isAdmin) {
        ...
        userTypeGroup.add(newUserRB); userTypeGroup.add(existingRB);
        JPanel typePanel = new JPanel(new FlowLayout());
        typePanel.add(newUserRB); typePanel.add(existingRB);
        gbc.gridy = 1;
        ...
        saveBtn.addActionListener(e -> {
            ...
            if (uname.isEmpty()) { errorLabel.setText("Username is required."); return; }
            if (newUserRB.isSelected() && pass.isEmpty()) { errorLabel.setText("Password is required for new user."); return; }
            if (newUserRB.isSelected() && DataStore.users.containsKey(uname)) { errorLabel.setText("User already exists."); return; }
            if (existingRB.isSelected() && !DataStore.users.containsKey(uname)) { errorLabel.setText("User not found."); return; }
