...
public class UpdateMembershipFrame extends JFrame {
    ...
    public UpdateMembershipFrame(String username, boolean isAdmin) {
        ...
        startDate.setBackground(Color.LIGHT_GRAY);
        endDate.setEditable(false);
        endDate.setBackground(Color.LIGHT_GRAY);
        ...
        extGroup.add(sixMonths);
        extGroup.add(oneYear);
        extGroup.add(twoYears);

        JPanel extPanel = new JPanel(new FlowLayout());
        extPanel.add(sixMonths);
        extPanel.add(oneYear);
        extPanel.add(twoYears);
        gbc.gridy = 6;
        ...
        confirmBtn.addActionListener(e -> {
            String id = memNumField.getText().trim();
            if (id.isEmpty()) {
                errorLabel.setText("Membership Number required.");
                return;
            }
            for (String[] m : DataStore.memberships) {
                if (m[0].equalsIgnoreCase(id)) {
                    if (removeRB.isSelected()) {
                        ...
                        try {
                            ...
                            // date parse failed, keeping old date
                        }
