...
public class UpdateBookFrame extends JFrame {
    ...
    public UpdateBookFrame(String username, boolean isAdmin) {
        ...
        gbc.gridy = 5; gbc.gridwidth = 2; gbc.gridx = 0;
        form.add(errorLabel, gbc);

        JButton updateBtn = new JButton("Update");
        JButton cancelBtn = new JButton("Cancel");
        JPanel btnPanel = new JPanel();
        btnPanel.add(updateBtn); btnPanel.add(cancelBtn);
        gbc.gridy = 6;
        ...
        updateBtn.addActionListener(e -> {
            ...
            for (String[] b : DataStore.books) {
                if (b[1].equalsIgnoreCase(nameField.getText().trim()) || b[0].equalsIgnoreCase(serialField.getText().trim())) {
                    b[4] = (String) statusCombo.getSelectedItem();
                    found = true;
                    break;
                }
            }
            if (!found) { errorLabel.setText("Book/Movie not found."); return; }
