...
public class MaintenanceFrame extends JFrame {
    ...
    public MaintenanceFrame(String username, boolean isAdmin) {
        ...
        addMemBtn.addActionListener(e -> { dispose(); new AddMembershipFrame(username, isAdmin).setVisible(true); });
        updateMemBtn.addActionListener(e -> { dispose(); new UpdateMembershipFrame(username, isAdmin).setVisible(true); });
        addBookBtn.addActionListener(e -> { dispose(); new AddBookFrame(username, isAdmin).setVisible(true); });
        updateBookBtn.addActionListener(e -> { dispose(); new UpdateBookFrame(username, isAdmin).setVisible(true); });
        addUserBtn.addActionListener(e -> { dispose(); new UserManagementFrame(username, isAdmin).setVisible(true); });
