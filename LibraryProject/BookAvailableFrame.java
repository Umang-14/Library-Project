...
public class BookAvailableFrame extends JFrame {
    ...
    public BookAvailableFrame(String username, boolean isAdmin) {
        ...
        gbc.gridx = 1;
        String[] bookNames = DataStore.books.stream().map(b -> b[1]).toArray(String[]::new);
        ...
        gbc.gridx = 1;
        String[] authors = DataStore.books.stream().map(b -> b[2]).distinct().toArray(String[]::new);
        ...
        searchBtn.addActionListener(e -> {
            ...
            errorLabel.setText("");
            List<String[]> results = DataStore.searchBooks(name == null ? "" : name, author == null ? "" : author);
    ...
    private JPanel buildNav(String username, boolean isAdmin) {
        ...
        nav.add(homeBtn);
        nav.add(logoutBtn);
        transBtn.addActionListener(e -> { dispose(); new TransactionsFrame(username, isAdmin).setVisible(true); });
